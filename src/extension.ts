...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        openDetailedDemoCommand, 
        codeLensProviderDisposable,
        hoverProviderDisposable
    );
}

class PonderCodeLensProvider implements vscode.CodeLensProvider {
...
class PonderHoverProvider implements vscode.HoverProvider {
    private parsePonderComment(document: vscode.TextDocument, startLine: number): { demo: PonderDemo | null, endLine: number } {
        const line = document.lineAt(startLine);
    ...
    provideHover(
        ...
        if (result.demo) {
            ...
            if (position.line >= checkLine && position.line <= result.endLine) {
                ...
                if (commentMatch) {
                    ...
                    hoverRange = new vscode.Range(
                        checkLine, 0,
                        result.endLine, document.lineAt(result.endLine).text.length
                    );
                }
            }
            ...
            if (isOverComment) {
                ...
                const previewSize = config.get<number>('hoverPreviewSize', 200);
